	...
	case 'kbbi':
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 MARCANDO TODO MUNDO QUEM NN GOSTA QUE SE FODA🤬💪 〙✪══\n╠➥'+teks+'╚═〘 bot so jaozin 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
	    ...
	    case 'kapankah':
			...
			case 'closegc':
					...
					var nomor = mek.participant
					const close = {
					text: `Grupo fechado pelo admemir @${nomor.split("@s.whatsapp.net")[0]}\nagora *apenas gays* podem enviar mensagens`,
					contextInfo: { mentionedJid: [nomor] }
					}
               ...
               if (anu.error) return reply(anu.error)
				...
				if (isUser) return reply(mess.only.daftarB)
					...
					break
                                case 'nsfwneko':
				    try{
						if (!isNsfw) return reply('❌ *NSFW NAUM ATIVADO*digite .nsfw 1 para ativar ❌')
                                                if (!isUser) return reply(mess.only.daftarB)
